...
logging.basicConfig(filename=f'{outpath}{dataset}{sep}{dataset}-test_linear_HalvingGridSearchCV_DEBUG02.log', level=logging.DEBUG)
...
def train_single_model(train_data, test_data, y_train, y_test, dataset, estimator, cv=5):
    sub_start_time = dt.datetime.now()
    if estimator=='svm':
        est = SVC(kernel='linear', C=1)
    elif estimator=='rf':
        est = RandomForestClassifier(random_state=42, n_estimators=100, max_depth=5, min_samples_leaf=2, min_samples_split=2)
    elif estimator=='rr':
        est = RidgeClassifier(alpha=1e-1)
    logging.info(f"Training {estimator.upper()} model for {dataset.upper()} data...")
    meta_dict = {
        'data_uid':run_uid,
    ...
    est.fit(train_data, y_train)
    predictions = est.predict(test_data)
    test_accuracy = est.score(test_data, y_test)
    training_accuracy = est.score(train_data, y_train)
...
for method in method_test_list:#subset_df['method']:
    ...
    cv_results = train_single_model(train_x, test_x, train_y, test_y, dataset, 'svm', cv = gkfv)
    cv_results = train_single_model(train_x, test_x, train_y, test_y, dataset, 'rf', cv = gkfv)
    cv_results = train_single_model(train_x, test_x, train_y, test_y, dataset, 'rr', cv = gkfv)
