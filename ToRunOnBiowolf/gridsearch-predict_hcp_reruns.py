...
from sklearn.linear_model import RidgeClassifier, Lasso, ElasticNet, SGDClassifier
...
logging.basicConfig(filename=f'{outpath}{dataset}{sep}__{dataset}-GridSearchCV-ACTUAL_RERUN_{job_id}.log', level=logging.DEBUG)
...
def train_models_with_gridsearch(train_data, test_data, y_train, y_test, dataset, cv=5):
    ...
    for name, config in model_configs.items():
        ...
        logging.info(f"{name.upper()} model for {dataset.upper()} data complete!")
        logging.info(f'Finished: {dt.datetime.now()}')
...
rerun_list = [
    'Hierarchical-1', 'Random_19877_v0', 'Random_19877_v1', 'Random_19877_v2', 'Random_19877_v3', 'Random_19877_v4', 'Random_19877_v5', 'Random_19877_v6', 'Random_19877_v7', 'Random_19877_v8', 'Random_19877_v9', 'Random_19900_v0', 'Random_19900_v1', 'Random_19900_v2', 'Random_19900_v3', 'Random_19900_v4', 'Random_19900_v5', 'Random_19900_v6', 'Random_19900_v7', 'Random_19900_v8', 'Random_19900_v9'
]

method = rerun_list[array_index]
# try:
...
if method == 'All':
    ...
    try:
        ...
        train_x.loc[:,'Subject'] = feature_set_dict['train_x'].loc[:,'Subject']
        train_y = feature_set_dict['train_y'].values.ravel()
        test_x = feature_set_dict['test_x'][feature_set]
        test_x.loc[:,'Subject'] = feature_set_dict['test_x'].loc[:,'Subject']
        test_y = feature_set_dict['test_y'].values.ravel()
    except Exception as e:
        sub_end_time = dt.datetime.now()
    ...
    # n_features = sval[1]
    # version = sval[2]
    selected_vars = np.load(f'{fs_outpath}{k}{sep}{run_uid}_{method}.npy', allow_pickle=True)
